...
require(["jquery","echarts","chinaMap","worldMap","bmap","data"],function($,echarts,china,world,bmap,datas){
   // console.log($)
    var chartOne= echarts.init($('#china')[0])
    var chartTwo= echarts.init($('#leida')[0])
    var chartThree= echarts.init($('#scatter')[0])
    var chartFour = echarts.init($('#four')[0]);
    var chartFive = echarts.init($('#five')[0]);
    var chartSix = echarts.init($('#six')[0]);
    var chartSeven = echarts.init($('#seven')[0]);
    var chartEight = echarts.init($('#eight')[0]);
    var main = echarts.init($('#mainM')[0]);
...
//optionFive
    ...
    setInterval(function (){
        optionFive.series[0].data[0].value = (Math.random()*100).toFixed(2) - 0;
        optionFive.series[1].data[0].value = (Math.random()*7).toFixed(2) - 0;
        optionFive.series[2].data[0].value = (Math.random()*2).toFixed(2) - 0;
        optionFive.series[3].data[0].value = (Math.random()*2).toFixed(2) - 0;
...
//optionnine
   ...
   // chartSix.setOption(optionSix);
    ...
    chartEight.setOption(optionEight);
    main.setOption(optionMain)
    //加载慢
    BeijingBus(datas.busData.busData)
})
